...
Object.defineProperty(t,"__esModule",{value:!0}),t.zi=t.ru=t.Er=t.Ti=t.Pi=t.Ri=t.Ne=void 0,t.Ne=e=>{
let u,n,f=e.u,s=f.toLowerCase(),a=null,o=!1;if(!i.an.test(r.mi(s)))return i.Gl(),null;if(e.p){let l=t.Ri(e);return{k:"",
s:0,u:l.p!=null?l.u:f,e:l.p!=null?l.p:l.u}}for(u of((n=i.qr(s))&&(s=s.slice(n),f=f.slice(n)),
l.Hl.rules))if(s.startsWith(u.qi)&&(a=f.slice(u.qi.length).match(u.Ui),a))break;if(!a||!u){let e=l.Ze.ui
...
a.length>1&&!u.Ui.global&&a.shift();let p=u.Ai,g=0;if(a.length>1)o=!0;else if(p instanceof RegExp)s=a[0],
(a=s.match(p))?(a.shift(),o=!0):a=[s];else if(p==" "||p==="+"||p instanceof Array){s=a[0].toLowerCase()
;let t=s.indexOf(":");if(t=t>0&&t<s.length?t:0,t&&!i.an.test(s)&&!s.startsWith("file:")){let e=r.di(s,t,s.indexOf(" "))
;t=e!==-1&&e<=3?t:0}if(g=t>0?s.startsWith("data:")?2:1:0,g)a=[a[0]];else{let t=typeof p=="object",l=t?p[0]:p
;if(t&&l==="+"){let t=u.qi.length;u.Ui.global||(t+=a.index),t=n+t+Math.max(0,f.slice(t).indexOf(a[0])),l=r.fi(e.u,t),
...
c:m,u:s,s:o?s.lastIndexOf(" ")+1:0}},t.Ri=e=>{let{u:t}=e,n=t.toLowerCase();if(e.p===1){let i=l.S(t,131072,e.s)
;if(i!==t&&i&&i!==t+"/"&&i+"/"!==t){let e=r._r(i,null,-2);if(r.dr===0)return{u:e,p:"(sed)"}}}
if(!i.an.test(r.mi(n)))return{u:"This url has no upper paths",p:null}
...
;if(u&&u.length<l.length&&(i._i(u,!0)||"%-".includes(l[u.length])))return e.slice(0,t+f-4)+e.substr(t+f,u.length)}
...
l&&",.;\u3002\uff0c\uff1b".includes(l[0])&&(l=l.slice(1).trim()),o&1&&l&&a.test(l[0])&&(l=l.slice(1)),r.pi(),
...
;if(d=d.replace(/\\s/g," ").trim().replace(/([^\\]|^)%(s)/gi,"$1$$$2").replace(/\\%/g,"%"),o={Ir:"",Jl:d,u:h,Hi:l.size>1
},u.includes("~")&&o.Hi&&(r._r(d,null,-2),r.dr>2&&(u=u.filter(e=>e!=="~"))),u=u.filter($),u.length!==0){if(p===-1){
for(r.hi.lastIndex=0;(g=r.hi.exec(d))&&g[0].endsWith("$"););g&&(p=g.index+1)&&(a=g[2],
a?(a=/^s:/i.test(a)?a[0]==="s"?"+":" ":a,
s=""):(a=g[1]==="s"?"+":" ",s=[a]),d=d.replace(r.hi,(e,t)=>"$"+(t||"s")).toLowerCase(),d=r._r(d,null,-1),
r.dr>2&&(d=d.replace(/%24(%24|s)/g,decodeURIComponent)),p=0,d=d.replace(/\$[$s]/g,(e,t)=>e==="$$"?(p>0||p--,
"$"):(p=p>0?p:p+t+1,e)),(n=f(d,p,a))&&(a.includes("$")?(a=a.replace(r.bi,e=>e==="$$"?"\\$":"(.*)"),
s=new RegExp("^"+a,/[a-z]/i.test(a)?"i":"")):s=s||a.trim()||" ",c.push({qi:n.qi,Ui:n.Ui,Ir:u[0].trimRight(),Ai:s})))
...
p=e.search(m);let r=i.Tl(d,l?p>=0?e.slice(0,p):e:"");r&&(a=t.zi(a),c.push({qi:a,Ui:r,Ir:u[0].trimRight(),
Ai:[o.Jl.lastIndexOf("$S")>=0?" ":"+"]})),e=p>=0?e.slice(p+1):""}else e=e.slice(p+4);e=e.trimLeft(),
...
;return f=f&&i.hl(f).replace(/\\\+|%20| /g,"(?:\\+|%20| )"),u=t.zi(u),{qi:u,Ui:new RegExp(n+f+e,/[a-z]/i.test(f)?"i":"")
}};t.zi=e=>{let t=e.slice(0,9).toLowerCase(),l=i.qr(t)
