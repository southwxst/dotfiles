...
}),t.oi=t.fn=t.si=t.Kl=t.fi=t.K=t.gn=t.mi=t.di=t._r=t.pi=t.ci=t.dr=t.wi=t.gi=t.$i=t.bi=t.hi=t.es=t.vi=t.xi=void 0,
t.xi=/^([^:]+(:[^:]+)?@)?([^:]+|\[[^\]]+])(:\d{2,5})?$/,t.vi=/^(?:ext|web)\+[a-z]+:/,
t.es=/^"[^"]*"$|^'[^']*'$|^\u201c[^\u201d]*\u201d$/,t.hi=/\$([sS$])?(?:\{([^}]*)})?/g,t.bi=/\$([+-]?\d+|\$)/g,
t.$i=/^[\w\x80-\ufffd]{1,8}!?>/,t.gi=/<[\w\x80-\ufffd]{1,8}!?$/,t.wi=/\|([\w\x80-\ufffd]{1,8}|(,|%2[cC])[\w,-]*)$/
...
"release-notes":"/RELEASE-NOTES.md",setting:n,settings:n,wiki:"/wiki"};t.dr=0,t.ci=!1,t.pi=()=>{t.dr=0},
t._r=(e,i,n,s)=>{if(e=e.trim(),t.dr=0,l.Ei(e))return e=e.replace(/\xa0/g," "),l.Gl(),e;let o,m,d,p,c=-1,w=0,g=!1
...
n<-1||!e?d="vimium://"+e:n===-1||$||!(d=r.Fr(e,n,null,(s||0)+1))?d=t.gn(e,!1,n):typeof d!="string"&&(c=5)):t.vi.test(e)?c=0:((m=e.indexOf("/",o+3))===-1?e.length<d.length:e.charCodeAt(m+1)<33)?c=4:/[^a-z]/.test(e.slice(0,o))?c=(o=e.charCodeAt(o+3))>32&&o!==47?0:4:e.startsWith("file:")?c=0:e.startsWith("chrome:")?c=e.length<d.length&&e.includes("/")?4:0:r.el&&e.startsWith("read:")?c=!/^read:\/\/([a-z]+)_([.\da-z\-]+)(?:_(\d+))?\/\?url=\1%3a%2f%2f\2(%3a\3)?(%2f|$)/.test(e)||e.length<d.length?4:0:e=e.slice(o+3,m>=0?m:void 0):(o!==-1&&e.lastIndexOf("/",o)<0&&(c=t.di(d.toLowerCase(),o,e.length%d.length)),
w=2,
...
c!==-1||((p=t.xi.exec(e))?(e=p[3]).endsWith("]")?c=l.kn(e,6)?w:4:e==="localhost"||e.endsWith(".localhost")||l.kn(e,4)||p[4]&&g?c=w:(o=e.lastIndexOf("."))<0||(c=l._i(e.slice(o+1),!1,e))===0?(m=e.length-o-1,
c=w!==2&&(o<0||(w!==0?m>=3&&m<=5:m>=2&&m<=14)&&!/[^a-z]/.test(e.slice(o+1)))||u(e,p[4])>0?w:4):c=/[^.\da-z_\-]|xn--|^-/.test(e)?e.startsWith("xn--")||e.includes(".xn--")||(e.length===o+3||c!==1?!w:u(e,p[4]))?w:4:w!==2||g?w:e.endsWith(".so")&&e.startsWith("lib")&&e.indexOf(".")===e.length-3?4:p[2]||p[4]||!p[1]||/^ftps?(\b|_)/.test(e)?2:e.startsWith("mail")||e.indexOf(".mail")>0||(m=e.indexOf("."))===o?4:e.indexOf(".",++m)!==o?2:e.length===o+3&&c===1&&l._i(e.slice(m,o),!0)?4:2:(c=4,
e.length===d.length&&l.kn(e=`[${e}]`,6)&&(d=e,c=2))),l.Gl(),s||(t.ci=!1),t.dr=c,
...
;if(l)return`${l.Yt?"https":"http"}://www.${e.toLowerCase().replace("/","")}/`}return""};t.di=(e,r,i)=>{
...
return/\d/.test(e)?0:4;default:return t.vi.test(e)?0:n?4:-1}},t.mi=e=>{
...
(u=s[f])!=null)(f==="release"||f==="releases")&&(u+="#v"+r.Ze.To.replace(/\D/g,"")),
...
;return n||(t.ci=!!s&&l!=="~"),l!=="~"?t._r(u,null,i,(n||0)+1):(t.dr=4,u)
},t.fi=(e,t)=>e.lastIndexOf("://",21)<0||l.Ei(e)||e.startsWith("vimium://run")||e.startsWith("data:")||!/\?|#.*=/.test(e.slice(0,t))?"%20":"+",
t.Kl=(e,i,n,s)=>{let u,a=0;return i=e.length===0&&n?n:i.replace(t.hi,(n,o,f,m)=>{let d
;if(n.endsWith("$")||!o&&!f)return"$";o||(/^s:/i.test(f)?(o=f[0],f=f===null||f===void 0?void 0:f.slice(2)):o="s")
;let p=e,c=f?t.wi.exec(f):null;c&&f.charAt(c.index-1)!=="\\"?f=f.slice(0,c.index):c=null
;let w=f?t.gi.exec(f)||t.$i.exec(f):null
...
p===e&&!u&&(u=d),o=t.fi(i,m)),f&&f.includes("\\")&&(f=f.replace(/\\([\\<>|])/g,"$1")),
f=d.length===0?"":f&&f.includes("$")?f.replace(t.bi,(e,t)=>{if(t==="$")return"$";let r=parseInt(t,10)
...
}),l.Gl(),s==null?i:{Jl:i,jr:s}},t.si=e=>{let r=e.indexOf(":"),l=r;if(r<=0)return e
;if(t.vi.test(e.slice(0,r+1).toLowerCase()))return e.slice(0,r).toLowerCase()+e.slice(r)
